...
class CrosswordCreator():
        ...
        if overlap:
            ...
            for val1 in self.domains[x].copy():
                for val2 in self.domains[y]:                   
                    if len(val1) > a and len(val2) > b:  
                        if val1[a] == val2[b] and val1 != val2:
                            flag = True
        ...
        if arcs == None:
            ...
            for key in self.crossword.overlaps:
                if overlaps[key] != None:
                    pos1 = key[0]
                    pos2 = key[1]
                    if (pos1, pos2) not in arcs and (pos2, pos1) not in arcs:
                        arcs.append(key)
        ...
        for var in assignment:
            ...
            for var2 in assignment:
                if var != var2:
                    ...
                    if overlap != None:
                        a,b = overlap
                        if assignment[var][a] != assignment[var2][b]:
                            return False
    ...
    def order_domain_values(self, var, assignment):
        ...
        for val1 in domain[var]:
            count = 0
            for var2 in neighbors:
                for val2 in domain[var2]:
                    overlap = self.crossword.overlaps[var, var2]
                    if overlap != None:
                        a,b = overlap
                        if val1[a] != val2[b]:
            ...
            least_constraints[val1] = count
        
        least_constraints = {key : val for key, val in sorted(least_constraints.items(), key = lambda item : item[1])}

        return list(least_constraints.keys())
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables:
            if var not in assignment:
                neighbors = self.crossword.neighbors(var)
                unassigned_variables.append([var, len(self.domains[var]), len(neighbors)])
